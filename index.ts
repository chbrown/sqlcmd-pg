...
export class Connection extends BaseConnection {
  ...
  */
  queryStream(sql: string, args: any[]) {
    this.emit('log', {level: 'info',
                      format: 'Creating query stream with SQL "%s" and variables: %j',
                      args: [sql, args]});

    const stream = new QueryStream(sql, args);
  ...
  */
  executeCommand<R>(command: Command<R>,
    ...
    sql = sql.replace(/\$[A-Za-z0-9_.]+/g, (match) => {
      ...
      const index = args.push(value);
      return '$' + index;
    });
    this.query(sql, args, callback as any);
  }
  ...
  */
  ...
  postgresConnection() {
    ...
    const options: pg.ConnectionConfig = {};
  ...
  */
  databaseExists(callback: (error: Error, exists?: boolean) => void) {
    const postgres_db = this.postgresConnection();
